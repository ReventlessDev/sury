...
function numberDecoder(input, selfSchema) {
  ...
  if (!(inputTagFlag & 2)) {
    if (inputTagFlag & 4) {
      if (input.s.format !== selfSchema.format && selfSchema.format === "int32") {
        refineInPlace(input, selfSchema, int32FormatValidation);
      }
      return input;
    } else {
...
function literalDecoder(input, selfSchema) {
  ...
  expected.const = ("" + selfSchema.const);
  input.validation = (inputVar, negative) => {
    input.e = expected;
    return inputVar + (
      negative ? "!==" : "==="
  ...
  let getArrItemsCode = (arr, isDeopt) => {
    ...
    while (itemIdx <= lastIdx) {
      ...
      try {
        ...
        if (itemOutput.i !== typeValidationInput.i) {
          if (itemOutput.f & 1) {
...
function jsonEncoder(input, to) {
  ...
  if (toTagFlag & 1024) {
    let expected = copySchema(string);
    expected.to = to;
    input.s = unknown;
    input.e = expected;
    return stringDecoder(input, expected);
  }
  if (toTagFlag & 2064) {
    input.s = unknown;
    input.e = nullLiteral;
  ...
  if (toTagFlag & 128) {
    ...
    let output = arrayDecoder(input, input.e);
  ...
  let output$1 = objectDecoder(input, input.e);
