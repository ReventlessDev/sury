...
function appendRefiner(existingRefiner, newRefiner) {
  if (existingRefiner !== undefined) {
    return (input, selfSchema) => {
      let output = existingRefiner(input, selfSchema);
      output.c = output.c + newRefiner(output, selfSchema);
      return output;
    };
  } else {
    return (input, selfSchema) => {
      input.c = input.c + newRefiner(input, selfSchema);
      return input;
    };
  }
}
...
function internalRefine(schema, refiner) {
  ...
  root.refiner = appendRefiner(root.refiner, refiner(root));
  return root;
}
