...
function parse$1(b, schema, inputArg) {
  ...
  input = schema.decoder(b, input, schema);
  let to = schema.to;
  if (to !== undefined) {
...
function appendRefiner(existingDecoder, refiner) {
  return (param, input, selfSchema) => {
    let output = existingDecoder(input, input, selfSchema);
...
function unionDecoder(b, input, selfSchema) {
  ...
  for (let idx = 0; idx <= lastIdx; ++idx) {
    ...
    let schema = target !== undefined && !selfSchema.parser ? updateOutput(schemas[idx], mut => {
        mut.to = target;
      }) : schemas[idx];
  ...
  o.s = to !== undefined && !selfSchema.parser ? (o.t = true, getOutputSchema(to)) : selfSchema;
  return o;
}
...
function getWithDefault(schema, $$default) {
  return updateOutput(schema, mut => {
    ...
    if (anyOf !== undefined) {
      ...
      to.decoder = (_b, input, param) => input;
      mut.to = to;
