...
let exposeMetadataId = `m:` + "sury" + `:` + "expose";

function expose(schema) {
  return set$1(schema, exposeMetadataId, true);
}

function isExposed(schema) {
  let match = schema[exposeMetadataId];
  if (match !== undefined) {
    return match;
  } else {
    return false;
