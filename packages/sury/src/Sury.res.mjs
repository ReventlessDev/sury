...
function symbolDecoder(input, selfSchema) {
  let inputTagFlag = flags[input.s.type];
  if (inputTagFlag & 1) {
    refineInPlace(input, selfSchema, (inputVar, negative) => "typeof " + inputVar + (
      negative ? "!==" : "==="
...
function literalDecoder(input, selfSchema) {
  ...
  let expected = base(stringTag, false);
...
function parse$1(input) {
  ...
  let output = encoder !== undefined && expected.type !== unknownTag ? encoder(input, expected) : input;
  if (output.t !== true) {
...
function recursiveDecoder(input, selfSchema) {
  ...
  let flag = input.g.o;
  let inputSchema = input.s.seq === selfSchema.seq ? def : input.s;
...
function jsonDecoder(input, selfSchema) {
  ...
  if (inputTagFlag & 64) {
    let mut$1 = base(objectTag, false);
    let properties = {};
